...
def check_repo_exists(git_data):
    if not int(git_data.get('total_count', 0)):
        print("{} not found in {} Github, exiting " \
...
def check_project_exists(url):
    def man_checker(manifest):
        try:
            man = ES.parse(manifest)
            man = man.getroot()
        except IOError, ES.ParseError:
            print("WARNING: error while parsing %s"
                      % manifest)
        else:
...
def parse_device_from_manifest(device):
    def man_checker(manifest):
        try:
            ...
            print("WARNING: there was an issue parsing {}".format(manifest))
        else:
    ...
    man_check = man_checker(".repo/manifest.xml")
    if man_check:
        return man_check
    return None
