...
def create_manifest_project(url, directory,
		remote=default_rem, revision=default_rev):
	...
	project = ES.Element("project", attrib = { "path": directory, 
		"name": url, "remote": remote, "revision": revision})
	return project
...
# walking a few folders is quicker than parsing multiple files
def parse_device_from_folder(device):
	...
	if len(search) > 1:
		print("multiple devices under the name %s. defaulting to checking the manifest" % device)
...
def parse_dependency_file(location):
	...
	dep_location = location + dep_file
	...
	for dependie in dependencies:
		...
		if remote == "github":
			if not "/" in repository:
				repository = "omnirom/" + repository
		project = create_manifest_project(repository, target_path,
...
def fetch_dependencies(device):
	...
	if not os.path.isdir(location):
		print("ERROR: could not find your device folder location, bailing out")
		sys.exit()
